          ...
          {(searchQuery || filterStatus !== 'all') && (
            <div className="mt-4 pt-4 border-t border-gray-200 flex items-center justify-between">
              <p className="text-sm text-gray-600">
                Showing <span className="font-semibold text-gray-900">{filteredOrders.length}</span> {filteredOrders.length === 1 ? 'order' : 'orders'}
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              {(filterStatus !== 'all' || searchQuery) && (
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-900 rounded-xl font-semibold transition-all shadow-sm"
                >
              ...
              <Link
                to="/products"
                className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white rounded-xl font-semibold transition-all shadow-lg hover:shadow-xl"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                </svg>
          ...
          <div>
            ...
            <div className="space-y-5">
              {paginatedOrders.map((order) => (
                <div key={order.id} className="bg-white rounded-2xl border border-gray-200 overflow-hidden hover:shadow-xl hover:border-green-200 transition-all duration-300">
                  {/* Desktop View */}
                  <div className="hidden md:block">
                    {/* Collapsible Header */}
                    <button
                      onClick={() => setExpandedOrderId(expandedOrderId === order.id ? null : order.id)}
                      className="w-full p-8 flex items-center hover:bg-gradient-to-r hover:from-green-50/50 hover:to-transparent transition-all duration-300"
                    >
                      <div className="flex-1">
                        {/* Header Row */}
                        <div className="grid grid-cols-12 gap-6 mb-6 pb-6 border-b border-gray-200">
                          <div className="col-span-3 text-left">
                            <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mb-1.5">Order Placed</p>
                            <p className="text-sm text-gray-900 font-semibold">{new Date(order.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</p>
                          </div>
                          ...
                          <div className="col-span-5 flex items-end justify-end">
                            <span className={`px-4 py-2 rounded-xl text-xs font-bold shadow-sm ${getStatusBadge(order.status)}`}>
                        ...
                        <div className="grid grid-cols-12 gap-6 items-center">
                          ...
                          <div className="col-span-3 flex gap-2.5">
                            {order.items?.slice(0, 3).map((item, idx) => (
                              <div key={idx} className="w-16 h-16 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl flex items-center justify-center overflow-hidden border-2 border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                                {item.imageUrl ? (
                            ...
                            {order.items?.length > 3 && (
                              <div className="w-16 h-16 bg-gradient-to-br from-green-50 to-green-100 rounded-xl flex items-center justify-center border-2 border-green-200 shadow-sm">
                                <span className="text-base font-black text-green-700">+{order.items.length - 3}</span>
                              </div>
                            )}
                          </div>
                    ...
                    {expandedOrderId === order.id && (
                      <div className="border-t border-gray-200 p-8 bg-gradient-to-br from-gray-50 to-white space-y-8">
                            ...
                            <div className="flex justify-between relative px-4">
                              ...
                              {['Pending', 'Processing', 'Shipped', 'Delivered'].map((step, index) => (
                                <div key={step} className="flex flex-col items-center z-10">
                                  <div
                                    className={`w-11 h-11 rounded-full flex items-center justify-center text-lg font-bold transition-all duration-300 border-4 ${ 
                                  ...
                                  <p className={`text-xs font-semibold mt-3 text-center ${getStatusStep(order.status) >= index ? 'text-gray-900' : 'text-gray-500'}`}>{step}</p>
                                </div>
                              ))}
                            </div>
                          </div>
                        ...
                        {/* Cancelled Notice */}
                        {order.status === 'Cancelled' && (
                          <div className="bg-gradient-to-br from-red-50 to-red-100 border-2 border-red-200 rounded-2xl p-6 shadow-sm">
                            <div className="flex items-center gap-3">
                              <div className="w-12 h-12 bg-red-200 rounded-full flex items-center justify-center flex-shrink-0">
                                <span className="text-2xl">❌</span>
                              </div>
                              <div>
                                <h4 className="text-base font-bold text-red-900 mb-1">Order Cancelled</h4>
                        ...
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
                          ...
                          <div className="space-y-3">
                            {order.items?.map((item, idx) => (
                              <div key={idx} className="flex justify-between items-center bg-gray-50 p-4 rounded-xl border border-gray-200 hover:border-green-300 hover:bg-green-50/50 transition-all">
                                <div className="flex items-center gap-4">
                                  <div className="w-14 h-14 bg-white rounded-lg flex items-center justify-center overflow-hidden border-2 border-gray-200 shadow-sm">
                                    {item.imageUrl ? (
                                      <img src={item.imageUrl} alt={item.name} className="w-full h-full object-contain p-1" />
                                    ) : (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="border-t-2 border-gray-200 pt-3 mt-3 flex justify-between items-center p-3 bg-gradient-to-r from-green-50 to-green-100 rounded-lg">
                                <span className="font-bold text-gray-900">Total</span>
                                <span className="font-black text-green-600 text-2xl">${order.total?.toFixed(2)}</span>
                              </div>
                            </div>
                          </div>
                            ...
                            <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
                              ...
                              <div className="p-4 bg-gray-50 rounded-lg">
                                <p className="text-sm text-gray-700 leading-relaxed">
                                  <span className="font-semibold text-gray-900 block mb-2">{order.customerInfo.fullName}</span>
                                  {order.customerInfo.address}<br />
                                  {order.customerInfo.city}, {order.customerInfo.postalCode}<br />
                                  <span className="font-medium text-gray-900 mt-2 inline-block">📞 {order.customerInfo.phone}</span>
                                </p>
                              </div>
                            </div>
                          )}
                        ...
                        <div className="flex gap-3 pt-4">
                          <button className="flex-1 text-green-600 hover:text-white hover:bg-green-600 font-semibold py-3 border-2 border-green-600 rounded-xl transition-all shadow-sm hover:shadow-md">
                            View Invoice
                          </button>
                          {order.status === 'Delivered' && (
                            <button className="flex-1 px-4 py-3 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white font-semibold rounded-xl transition-all shadow-md hover:shadow-lg">
                              Buy Again
                            </button>
                          )}
                        </div>
                  ...
                  <div className="md:hidden">
                    ...
                    {expandedOrderId === order.id && (
                      <div className="border-t border-gray-200 p-4 bg-gray-50 space-y-4">
                        ...
                        {order.status !== 'Cancelled' && (
                          <div>
                            <h3 className="font-semibold text-gray-900 mb-4 text-sm">Delivery Progress</h3>
                            <div className="flex justify-between relative px-2">
                              {/* Timeline Line */}
                              <div className="absolute top-4 left-2 right-2 h-0.5 bg-gray-200 z-0">
                                <div
                              ...
                              {['Pending', 'Processing', 'Shipped', 'Delivered'].map((step, index) => (
                                <div key={step} className="flex flex-col items-center z-10">
                                  ...
                                  <p className="text-[10px] font-medium text-gray-700 mt-2 text-center max-w-[60px]">{step}</p>
                                </div>
                              ))}
                            </div>
                          </div>
                          ...
                          <div className="bg-red-50 border border-red-200 rounded p-3">
                            <div className="flex items-start gap-2">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-red-800">This order has been cancelled.</p>
                              </div>
                            </div>
                          </div>
                        )}
            ...
            {totalPages > 1 && (
              <div className="mt-10 flex items-center justify-center gap-2">
                <button 
                  ...
                  className="w-10 h-10 rounded-lg flex items-center justify-center text-gray-600 hover:text-green-600 hover:bg-green-50 transition-all disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-transparent"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
