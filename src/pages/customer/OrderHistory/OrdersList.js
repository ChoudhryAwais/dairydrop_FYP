            ...
            >
              <div className="w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6 items-center">
                  ...
                  {/* Mobile: Invoice + Buy Again */}
                  <div className="md:hidden w-full pt-3 flex items-center justify-end gap-2">
                    <button
                      onClick={(e) => { e.stopPropagation(); }}
                      className="text-green-600 hover:text-green-700 text-sm font-semibold flex items-center gap-2 transition-colors"
                    >
                    ...
                    {!(order.status && order.status.toLowerCase() === 'cancelled') && (
                      <button
                        onClick={(e) => { e.stopPropagation(); if (typeof onBuyAgain === 'function' && !processingBuyAgain) onBuyAgain(order); }}
                        disabled={processingBuyAgain}
                        className="px-3 py-1.5 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-lg text-sm font-semibold hover:from-green-700 hover:to-green-800 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                      >
                        {processingBuyAgain ? (
                          <>
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      {order.status === 'Delivered' && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            if (typeof onBuyAgain === 'function' && !processingBuyAgain) onBuyAgain(order);
                          }}
                          disabled={processingBuyAgain}
                          className="px-3 py-2 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-lg text-sm font-semibold hover:from-green-700 hover:to-green-800 transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                        >
                          {processingBuyAgain ? (
                            <>
                              <svg className="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24">
                    ...
                    <div className="bg-white rounded-xl md:rounded-2xl border border-gray-200 p-4 md:p-6 shadow-sm">
                      ...
                      <div className="p-3 md:p-4 bg-gray-50 rounded-lg">
                        <p className="text-xs md:text-sm text-gray-700 leading-relaxed">
                          ...
                          <span className="font-medium text-gray-900 mt-1 md:mt-2 flex items-center gap-1">
                    ...
                    View Invoice
                      ...
                      {!(order.status && order.status.toLowerCase() === 'cancelled') && (
                        <button
                          ...
                          disabled={processingBuyAgain}
                          className="flex-1 px-4 py-2 md:py-3 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white font-semibold rounded-lg md:rounded-xl transition-all shadow-md hover:shadow-lg text-sm md:text-base disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        >
                          {processingBuyAgain ? (
                            <>
